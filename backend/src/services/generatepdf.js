...
function addEducation(doc, data, y) {
    ...
    if (data.education?.internship?.institution) {
        const intern = data.education.internship;
        const dateRange = [
            intern.startDate ? intern.startDate.split('-')[0] : '',
            intern.endDate ? intern.endDate.split('-')[0] : ''
        ].filter(Boolean).join(' - ');
